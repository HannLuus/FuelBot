-- Automate B2B route access
-- 1. Add payment_method to b2b_subscriptions
-- 2. Allow "all routes" access when route_id is NULL for route_view
-- 3. Expose valid_until in get_my_b2b_entitlements

ALTER TABLE public.b2b_subscriptions
  ADD COLUMN IF NOT EXISTS payment_method text;

-- Return current user's active B2B entitlements including valid_until
-- and expanding route_id IS NULL to all active routes.
...
-- B2B route view: same visibility rule as hide unverified after 3 months, 
-- but allow access if route_id IS NULL for the route_view plan.
