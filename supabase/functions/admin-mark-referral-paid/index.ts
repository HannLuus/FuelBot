...
Deno.serve(async (req) => {
  ...
  if (updateRewardErr) {
    console.error('admin-mark-referral-paid reward update error:', updateRewardErr)
    return json({ error: 'Failed to update reward' }, 500)
  }

  const { error: updateStationErr } = await service
    .from('stations')
    .update({ referral_reward_status: 'PAID' })
    .eq('id', payload.station_id)

  if (updateStationErr) {
    console.error('admin-mark-referral-paid station update error:', updateStationErr)
  }
